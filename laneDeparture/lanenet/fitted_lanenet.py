...
parser.add_argument("-nn", "--nn_model", help="select model path for inference", default='lanenet_openvino_2021.4_6shave.blob', type=str)
...
src = np.float32([(230,165),
                  (282,165), 
                  (103,242), 
                  (409,242)])
dst = np.float32([(180,0),
                  (332,0),
                  (180,256),
                  (332,256)])
...
def process_image(img, mask, show=True):
    img_bin, _, Minv = unwarp(mask)
...
manip.inputImage.setQueueSize(5)
...
detection_nn.passthrough.link(passthroughOut.input)

# --------------- Inference ---------------
# Pipeline defined, now the device is assigned and pipeline is started
with dai.Device(pipeline) as device:
    #qManip = device.getOutputQueue(name="manip", maxSize=2)#, blocking=False)
    ...
    while True:
        ...
        try:
            # get passthrough frame
            manipFrame = qPass.get().getCvFrame()
        except Exception as e:
            print("can't get qPass")
            continue
        inNN = qNN.tryGet()
        
        if inNN is None:
            cv2.imshow("Lane segmentation", manipFrame)
            continue
        ...
        try:
            ...
            cv2.imshow("Lane segmentation", outFrame)
            print("preprocess")
            outFrame = process_image(manipFrame, clusters)
            print("postprocess")
        except Exception as e:
            print("can't cluster and show")
            outFrame = manipFrame
